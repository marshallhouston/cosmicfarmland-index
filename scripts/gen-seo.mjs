...
const llms = [
  ...
  ...section(
    'the farm',
    [
      `- [herbarium](${ORIGIN}/herbarium): every specimen the sheet mounts, named, with the garden bed it grows in.`,
...
// lastmod per url, from the last commit that touched the files that url is
// built from. One shared date made all 8 pages claim they changed on every
// deploy, and Google discounts a lastmod that moves when the page doesn't.
const SOURCES = new Map([
  [ORIGIN + '/', ['index.html', 'data/apps.json', 'data/catalog.json']],
  [ORIGIN + '/grayton', ['public/grayton.html']],
  [ORIGIN + '/herbarium', ['data/herbarium.json', 'public/specimens.json']],
...
const gitDate = (files) => {
  ...
  try {
    return (
      execFileSync('git', ['log', '-1', '--format=%cs', '--', ...files], {
        cwd: REPO,
...
// Railway builds from a source archive with no .git, so git answers nothing
// there. Reuse the dates already in the committed sitemap rather than stamping
// today: public/sitemap.xml is generated by `bun run gen` and committed, so
// those are the real per-page dates from the last run that could see git.
const committed = new Map()
try {
  const xml = readFileSync(join(REPO, 'public', 'sitemap.xml'), 'utf8')
  for (const m of xml.matchAll(/<loc>([^<]*)<\/loc><lastmod>([^<]*)<\/lastmod>/g))
    committed.set(m[1], m[2])
} catch {}

const BUILD_DATE = new Date().toISOString().slice(0, 10)
const lastmodFor = (u) => gitDate(SOURCES.get(u) ?? []) ?? committed.get(u) ?? BUILD_DATE
const entry = (u) => `  <url><loc>${u}</loc><lastmod>${lastmodFor(u)}</lastmod></url>`
const sitemap = [
